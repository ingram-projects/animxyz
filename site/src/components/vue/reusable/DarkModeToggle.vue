<template>
	<button
		...
		data-visitors-event="dark-mode-toggle"
	>
		<span class="screen-reader-only"
			>Turn Dark Mode {{ darkModeToggled ? "Off" : "On" }}</span
		>
...
export default {
	...
	mounted() {
		this.prefersColorSchemeMedia = window.matchMedia(
			"(prefers-color-scheme: dark)"
		);
		this.darkModeToggled = this.prefersColorSchemeMedia.matches;
		this.prefersColorSchemeMedia.addEventListener(
			"change",
	...
	beforeUnmount() {
		if (this.prefersColorSchemeMedia) {
			this.prefersColorSchemeMedia.removeEventListener(
				"change",
				this.onPrefersColorSchemeChange
			);
		}
