		...
		<table class="utilities-table">
			<thead>
				<tr>
					...
					<th
						class="level__header"
						v-for="utilityLevel in utilityLevels"
						:key="utilityLevel"
					>
						{{ utilityLevel }}
					</th>
			...
			>
				<tr
					...
					v-for="utility in computedUtilities"
					:key="utility.name"
				>
					...
					>
						<div class="utility-level__content" v-if="utilityLevel.valid">
							...
							<label
								class="toggle-label"
								:for="utilityLevel.id"
								data-visitors-event="sandbox-utility-select"
								data-visitors-event-value="{{ utilityLevel.string }}"
							>
...
export default {
	...
	computed: {
		utilityObjs() {
			return this.utilities.map((u) => getXyzUtility(u));
		},
		utilityLevels() {
			const levelsMap = {};
			...
			return this.utilityObjs.map((utility) => {
				return {
					...
					levels: this.utilityLevels.map((level) => {
						...
						return {
							...utilityLevel,
							id: `${this.uid}_${utilityLevel.string}`,
						};
					}),
				};
			});
		...
		modelValue: {
			...
			handler(val, oldVal) {
				if (!isEqual(val, oldVal)) {
					...
					Object.keys(this.modelValue).forEach((utilityString) => {
						const utility = getXyzUtilityRegex(utilityString);
						if (utility) {
							const model = this.getUtilityModel(utility);
							this.toggledUtilities[model] = utilityString;
						}
					});
				}
			},
	...
	methods: {
		...
		onCellClick(cell, model) {
			if (this.toggledUtilities[model] === cell.string) {
				delete this.toggledUtilities[model];
			}
		},
	},
};
