<template>
  <div class="page-nav__wrap" :class="{ open }" v-click-outside="onClickOutside">
    ...
    <XyzTransition
      ...
      v-xyz="{ 'left-100%': above('tablet'), 'down-100%': below('tablet') }"
    >
      <nav class="page-nav" v-show="open">
        <div class="nav-list__wrap" v-scroll-lock="below('tablet') && open">
          <ul class="nav-list" xyz="fade left-3 stagger-0.5">
            ...
            >
              ...
              <a
                ...
                v-if="!section.header"
                :href="`#${section.id}`"
                :ref="(el) => setActiveLinkRef(el, section)"
              >
                <div class="link-dot__wrap">
...
import AnimXyzLogo from '~/components/vue/reusable/AnimXyzLogo.vue'
...
export default {
        ...
        if (this.activeSection) {
          nextTick(() => {
            if (this.activeLinkEl) {
              this.activeLinkEl.scrollIntoView({ block: 'center' })
            }
          })
        }
      },
  ...
  methods: {
    ...
    setActiveLinkRef(el, section) {
      if (el && section === this.activeSection) {
        this.activeLinkEl = el
      }
    },
  },
}
