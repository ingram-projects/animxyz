  ...
  - name: Origin
    template: |
      <div class="example-wrap">
        <XyzTransition data-xyz="small-100%" v-xyz="data.utilities" :style="data.variables" v-on="data.listeners">
    ...
    code:
      - name: HTML
        content: |
          ##html
          <div class="square ${data.mode}" data-xyz="small-100%${data.utilitiesString && ' ' + data.utilitiesString}"></div>
      ...
      - name: Vue
        content: |
          ##vue
          <XyzTransition data-xyz="small-100%${data.utilitiesString && ' ' + data.utilitiesString}">
            <div class="square" v-if="${data.toggled}"></div>
          </XyzTransition>
...
For example setting `data-xyz="small-100% origin-right"` on an element will scale it to its right center edge.
